(function() {
  ...
  _.reject = function(collection, test) {
    ...
    return _.filter(collection, function(colVal){
      if (test(colVal)) {
        return false;
      } else {
        return true;
  ...
  _.uniq = function(array) {
    var arr = [];
    _.each(array, function(colVal){
      if (_.indexOf(arr, colVal) === -1) {
        arr.push(colVal);
      }
    });
    return arr;
  ...
  _.extend = function(obj) {
    for (var i = 1; i < arguments.length; i++) {
      _.each(arguments[i], function(colVal, key){
        obj[key] = colVal;
      });
    }
    return obj;
